...
if __name__ == '__main__':
    ...
    Neg_Path = '../NineHospital_LN+main lesion/negative'
    Pos_Path = '../NineHospital_LN+main lesion/positive'
    ...
    df.to_csv('./T2_ROIFeature_TestingData.csv',index=False,sep=',')
    df = DF(T1_ROIFeature).fillna('0')
    df.to_csv('./T1_ROIFeature_TestingData.csv',index=False,sep=',')
    df = DF(T2_LNFeature).fillna('0')
    df.to_csv('./T2_LNFeature_TestingData.csv',index=False,sep=',')
    df = DF(T1_LNFeature).fillna('0')
    df.to_csv('./T1_LNFeature_TestingData.csv',index=False,sep=',')
